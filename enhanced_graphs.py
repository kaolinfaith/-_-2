...
from test_repository import TestDependencyGraph
...
class EnhancedDependencyGraph(DependencyGraph):
    ...
    def find_reverse_dependencies(self, target_package: str):
...
class EnhancedTestDependencyGraph(TestDependencyGraph):
    """Расширенный тестовый граф с поддержкой обратных зависимостей"""

    def __init__(self, repository_path: str, max_depth: int = 3, filter_substring: str = ""):
        super().__init__(repository_path, max_depth, filter_substring)
        self.reverse_finder = ReverseDependencyFinder(self)
